...
What it does **not** ship: persistent staging across process restart, catalog migration from `nc-starter-0.1` / `0.2`, or a memoryjs graph mutation DSL (durable writes are a host `onDurableWrite` callback).
...
JSON-UI (`@json-ui/core`, `@json-ui/react`, `@json-ui/headless`) validates and renders catalog-constrained trees. memoryjs (`@danielsimonjr/memoryjs`) holds durable knowledge-graph state; NC projects entities and relations into the React data model. `createPythonRepl` owns a persistent CPython worker for computation. `createLlmIntentHandler` may pass that REPL as `repl` and drive it with `python_exec`.
...
```
neural-computer/
├── src/ (TypeScript under src/; see DEPENDENCY_GRAPH for file counts)
...
Six components, two actions. Select is a native list of string options.
