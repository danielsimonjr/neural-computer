...
function generateMarkdown(files: ParsedFile[], modules: ModuleMap, stats: Statistics, circularDeps: CircularDependencyResult, matrix: DependencyMatrix): string {
  ...
  // The architecture-docs gate (repo_map.py check) fails any doc that has
  // no '## Verification' section. This report holds no hand-written claim
  // to verify, so it declares the opt-out. The GENERATOR must write the
  // marker: a hand-added one survives only until the next run, and the
  // gate then fails a full cycle later, looking like a new defect.
  lines.push('<!-- repo-map:no-verification -->');
  lines.push('<!-- DO NOT EDIT: generated by tools/create-dependency-graph. Run `bun run docs:deps`. -->');
  lines.push('');
...
function generateTestCoverageMarkdown(coverage: TestCoverageAnalysis): string {
  ...
  // See generateMarkdown: the generator, not a human, writes the gate
  // opt-out for a report that carries no hand-written claim.
  lines.push('<!-- repo-map:no-verification -->');
  lines.push('<!-- DO NOT EDIT: generated by tools/create-dependency-graph. Run `bun run docs:deps`. -->');
  lines.push('');
...
async function main(): Promise<void> {
  ...
  unusedReport += '<!-- DO NOT EDIT: generated by tools/create-dependency-graph. Run `bun run docs:deps`. -->\n\n';
  unusedReport += '# Unused Files and Exports Analysis\n\n';
